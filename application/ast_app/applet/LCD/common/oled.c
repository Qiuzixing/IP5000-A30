...
/*
    chr: chr在char_list_normal[][8]中位置,char_list_normal[][8]是字模数组
*/
...
void show_a_char(u8 x, u8 y, u8 chr, u8 cursor, u8 mode) 
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {   
            ...
            {
                if (cursor == 1)
                    word[n] |= (char_list_normal[66*2+i][n] | char_list_normal[offset+i][n]); 
                else
                    word[n] |= (char_list_normal[offset+i][n]);
            }
            else
            {
                if (cursor == 1)
                    word[n] |= (char_list_inversion[66*2+i][n] | char_list_inversion[offset+i][n]); 
                else
                    word[n] |= (char_list_inversion[offset+i][n]);
            }
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        for (t = 0; t < lenth; t++)
        {
            ...
            {   
                if (mode == 1)
                {
                    word[count++] |= char_list_normal[offset+i][n];
                }
                else
                {
                    word[count++] |= char_list_inversion[offset+i][n];
                }
            }
        }
        //一次写多个字节,在这里写128
...
void OLED_Init(void)
{
    ...
    //OLED_WR_Byte(0x23,OLED_CMD); //呼吸灯
    //OLED_WR_Byte(0x00,OLED_CMD);
