...
static P3KReqistMsg_S* P3K_GetReqistMsgByID(int id)//Í¨¹ýID»ñÈ¡Handle
{
...
RESET_Udp:
	...
	myaddr.sin_addr.s_addr = 0;//Í¨ÅäµØÖ·  ½«ËùÓÐµÄµØÖ·°ó¶¨
	int opt = 1;  
    // sockfdÎªÐèÒª¶Ë¿Ú¸´ÓÃµÄÌ×½Ó×Ö  
...
void * P3K_UdpInsideServer()
{
	...
	myaddr.sin_addr.s_addr = inet_addr("127.0.0.1");//Í¨ÅäµØÖ·  ½«ËùÓÐµÄµØÖ·°ó¶¨
	int opt = 1;  
    // sockfdÎªÐèÒª¶Ë¿Ú¸´ÓÃµÄÌ×½Ó×Ö  
	...
	{  
		memset(recvBuf,0,sizeof(recvBuf));
        memset(userDefine,0,sizeof(userDefine));
...
long fun(char *s)
{
	int i,t;             //t¼ÇÂ¼ÁÙÊ±¼ÓµÄÊý
	long sum =0;
	for(i=0;s[i];i++)
		{
		if(s[i]>='0'&&s[i]<='9'){
			t=s[i]-'0';     }  //µ±×Ö·ûÊÇ0~9Ê±±£³ÖÔ­Êý²»±ä
		if(s[i]>='a'&&s[i]<='z'){
				t=s[i]-'a'+10;}
		if(s[i]>='A'&&s[i]<='Z'){
				t=s[i]-'A'+10;}
